...
def search(keyword):
    list_of_articles = article_titles()
    article_titles_containing_keyword = []
    if not keyword:
        return article_titles_containing_keyword
    for article in list_of_articles:
        if keyword.lower() in article.lower():
            article_titles_containing_keyword.append(article)
...
def title_length(max_length, titles):
    return [title for title in titles if len(title) <= max_length]
...
def multiple_keywords(keyword, titles):
    list_of_articles = search(keyword)
    for title in list_of_articles:
